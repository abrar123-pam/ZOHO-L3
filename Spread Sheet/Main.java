...
//        sheet1.addRow();
        ...
        System.out.println("\nFiltered Data:");
        printFilteredData(sheet1, filteredRows);
    ...
    private static void printSheetData(Sheet sheet) {
        ...
        for (int row = 0; row < numRows; row++) {
            printRowData(sheet, row);
        }
    }

    private static void printFilteredData(Sheet sheet, List<Row> filteredRows) {
        int numCols = sheet.getNumberOfColumns();

        printTableHeader(numCols);

        for (Row row : filteredRows) {
            printRowData(sheet, sheet.getRows().indexOf(row));
        }
    }

    private static void printTableHeader(int numCols) {
        System.out.print("      ");
    ...
    private static void printRowData(Sheet sheet, int rowIndex) {
        System.out.print(String.format("Row %-2d ", rowIndex));

        int numCols = sheet.getNumberOfColumns();
        for (int col = 0; col < numCols; col++) {
            try {
                Object data = sheet.getCellData(rowIndex, col);
